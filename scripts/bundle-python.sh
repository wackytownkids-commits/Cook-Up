...
echo "Installing extra deps some audiocraft versions miss..."
"$PY" -m pip install torchmetrics protobuf huggingface_hub tqdm omegaconf jsonschema soundfile

echo "Installing torchcodec (needed by newer torchaudio save paths)..."
"$PY" -m pip install torchcodec || echo "  (torchcodec not available - soundfile will be used for WAV saves)"

echo "Installing Flask (Cookup's local server)..."
"$PY" -m pip install "flask>=3.0"

echo "Installing xformers (with stub fallback if wheel unavailable)..."
if ! "$PY" -m pip install xformers 2>/dev/null; then
  echo "  xformers wheel not available - creating stub so audiocraft imports succeed."
  XF="$DEST/lib/python3.11/site-packages/xformers"
  mkdir -p "$XF/ops" "$XF/components"
  cat > "$XF/__init__.py" <<'PY'
__version__ = "0.0.0-stub"
PY
  cat > "$XF/ops/__init__.py" <<'PY'
def memory_efficient_attention(*a, **k):
    raise NotImplementedError("xformers stubbed - audiocraft uses native attention")
PY
  : > "$XF/components/__init__.py"
  cat > "$XF/components/attention.py" <<'PY'
class Attention:
...
class AttentionMask:
    def __init__(self, *a, **k):
        raise NotImplementedError()
PY
  echo "  Stub xformers created at $XF"
fi
